...
function plotArray(ctx, array, arrayX, color, minval, maxval, linewidth) {
    ...
    //x = 0;
	x = valToPixForX(arrayX[0], ctx, minval, maxval);
    ...
    for (k=1; k<70; ++k) {
		...
		x = valToPixForX(arrayX[k], ctx, minval, maxval);
        y = valToPix(array[k], ctx, minval, maxval);
		console.log(arrayX[k], x);
...
function valToPixForX(val, ctx, minval, maxval) {
